# OpenAI-Compatible API Server for Evil Hermes
...
SSE streaming. By exposing this endpoint, Evil Hermes becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  Evil Hermes   │
│  NextChat        │  │                                    │  gateway        │
...
4. Chat with Evil Hermes through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns Evil Hermes as a model) |
...
But Evil Hermes sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "evil-hermes"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured Evil Hermes model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of Evil Hermes's core prompt
...
Once implemented, Evil Hermes works as a drop-in backend for:
