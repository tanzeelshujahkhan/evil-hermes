...
<url>``) is spawned as a direct child of the Evil Hermes process. Evil Hermes's own
...
Evil Hermes process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Evil Hermes startup's opt-in
