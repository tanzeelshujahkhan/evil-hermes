...
Owns one Codex thread per Evil Hermes session. Drives `turn/start`, consumes
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Evil Hermes/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Evil Hermes session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="hermes",
            client_title="Evil Hermes Agent",
    ...
    def run_turn(
        ...
        *,
        ...
        into Evil Hermes' messages shape.
        ...
        # supports rich content but Evil Hermes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Evil Hermes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Evil Hermes' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Evil Hermes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Evil Hermes returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Evil Hermes version string for codex's userAgent line."""
    try:
