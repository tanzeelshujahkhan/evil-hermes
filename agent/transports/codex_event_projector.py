"""Projects codex app-server events into Evil Hermes' messages list.

The translator that lets Evil Hermes' memory/skill review keep working under the
...
Evil Hermes' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Evil Hermes
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Evil Hermes' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Evil Hermes stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Evil Hermes already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Evil Hermes' messages store text only.
        text_parts: list[str] = []
