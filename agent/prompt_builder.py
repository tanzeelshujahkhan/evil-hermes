...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Evil Hermes install /
    # source tree gain system-prompt authority. A backend that self-spawns
    # into that tree (the desktop app default) would otherwise load this
    # repo's contributor AGENTS.md as authoritative project context (#64590).
    # An explicitly configured cwd is honored verbatim — the Hermes tree is
    # a legitimate workspace when the user deliberately points a session at
    # it — and CLI/TUI surfaces pass allow_install_tree_fallback=True
    # because their launch dir IS the user's shell cwd.
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Evil Hermes install tree (%s) — set terminal.cwd to "
