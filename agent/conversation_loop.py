...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Evil Hermes tool use: "
    ...
    return (
        ...
        f"context, but Evil Hermes needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Evil Hermes config, set `model.ollama_num_ctx: 65536` "
...
def run_conversation(
    ...
    # all run inside Codex). Default Evil Hermes path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Evil Hermes internals.
        #
        # Evil Hermes invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Evil Hermes tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
            ...
            try:
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Evil Hermes/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # providers (DeepSeek, Kimi, MiMo, Evil Hermes).  A 429 can
                ...
                # which Evil Hermes' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Evil Hermes' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Evil Hermes may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Evil Hermes/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"timeout. (Evil Hermes's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Evil Hermes mid-turn, resume logic still sees the
