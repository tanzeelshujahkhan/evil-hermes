...
def init_agent(
    ...
    # Evil-Hermes rebrand — default flipped to True. Original hermes-agent
    # treats SOUL.md as opt-in (off by default, only loads when explicitly
    # requested via load_soul_identity=True). For Evil Hermes the SOUL.md
    # is the canonical identity source. It ships with the install and is
    # always loaded. Callers who want the legacy no-soul behavior can pass
    # load_soul_identity=False explicitly.
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Evil Hermes.  Choose a model with at least "
