...
row taxonomy is inferred from the atlas shape so Evil Hermes can render both legacy
...
# rows (1536x1872), while older Evil Hermes/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Evil Hermes' activity state names. They are not always identical to the
...
# Legacy Evil Hermes/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Evil Hermes activity names -> accepted row-name aliases in descending
