...
Plugins built on Evil Hermes Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Evil Hermes
...
embedded slugs, no shorthands. This mirrors Evil Hermes' main config
...
handles every provider, fallback chain, and per-task override Evil Hermes
supports.
"""
