...
DEFAULT_BOT_NAME = "Evil Hermes Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Evil Hermes Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Evil Hermes Telegram onboarding service.")
