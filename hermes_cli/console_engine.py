...
class HermesConsoleEngine:
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Evil Hermes Console does not run shell syntax. Use one supported "
                    "Evil Hermes command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Evil Hermes component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Evil Hermes logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Evil Hermes configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Evil Hermes configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        ...
                        f"{self.context} Evil Hermes Console."
                    )
        ...
        raise ConsoleCommandError(f"Unsupported Evil Hermes Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Evil Hermes Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in Evil Hermes Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Evil Hermes Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Evil Hermes Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Evil Hermes Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Evil Hermes Console.",
            ("skills", "publish"): "`skills publish` is not available in Evil Hermes Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Evil Hermes Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Evil Hermes Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Evil Hermes Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Evil Hermes Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Evil Hermes Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Evil Hermes Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Evil Hermes Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Evil Hermes Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Evil Hermes Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Evil Hermes Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Evil Hermes Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Evil Hermes Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Evil Hermes Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Evil Hermes Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Evil Hermes Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Evil Hermes Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Evil Hermes Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Evil Hermes Console.")
...
def run_console_repl(
    *,
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Evil Hermes Console. Type `help` for commands, `exit` to quit.", file=stdout)
