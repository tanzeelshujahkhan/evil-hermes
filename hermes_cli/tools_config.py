"""
Unified tool configuration for Evil Hermes.
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Evil Hermes routes X searches through xAI's built-in x_search "
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Evil Hermes process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in evil-hermes directory)")
            return
        ...
        _print_info("    Restart Evil Hermes for tracing to take effect.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Evil Hermes Tool Configuration", Colors.CYAN, Colors.BOLD))
