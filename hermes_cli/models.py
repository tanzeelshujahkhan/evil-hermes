...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Evil Hermes refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Evil Hermes that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Evil Hermes that doesn't ship
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    Evil Hermes is tool-calling-first — every provider path assumes the model
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without an Evil Hermes release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Evil Hermes will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Evil Hermes cannot verify the model name."
