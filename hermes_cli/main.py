...
Usage:
    ...
    hermes uninstall           Uninstall Evil Hermes Agent
...
def _print_fast_version_info() -> None:
    ...
    print(f"Evil Hermes Agent v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Evil Hermes Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Evil Hermes Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Evil Hermes Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Evil Hermes Agent...")
    print()
