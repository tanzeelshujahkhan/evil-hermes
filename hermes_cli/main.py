...
def _print_fast_version_info() -> None:
    ...
    print(f"Evil Hermes v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Evil Hermes'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall Evil Hermes (or just the Chat GUI with --gui)."""
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://evil-hermes.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    """Update Evil Hermes by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Evil Hermes — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Evil Hermes to the latest version.
    ...
    if is_managed():
        managed_error("update Evil Hermes")
        return
...
def cmd_acp(args):
    """Launch Evil Hermes as an ACP server."""
    try:
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://evil-hermes.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://evil-hermes.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
