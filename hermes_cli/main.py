...
Usage:
    ...
    hermes uninstall           Uninstall Evil Evil Hermes
...
def _print_fast_version_info() -> None:
    ...
    print(f"Evil Evil Hermes v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Evil Evil Hermes'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Evil Evil Hermes — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Evil Evil Hermes")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Evil Evil Hermes...")
    print()
