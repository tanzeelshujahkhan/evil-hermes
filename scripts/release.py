...
AUTHOR_MAP = {
    ...
    "hermes@nousresearch.com": "TanzeelShujahKhan",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/TanzeelShujahKhan/evil-hermes",
    ...
    lines.append(f"# Evil Hermes v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Evil Hermes. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Evil Hermes Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Evil Hermes Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Evil Hermes v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Evil Hermes v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Evil Hermes v{new_version} ({calver_date})' "
