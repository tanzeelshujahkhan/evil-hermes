  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://evil-hermes.local/docs' },
    ...
    expect(view.title).toBe('Failed to open evil-hermes.local')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://evil-hermes.local/docs' },
        result: { success: true, url: 'https://evil-hermes.local/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened evil-hermes.local')
  })
})
