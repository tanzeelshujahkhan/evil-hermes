# Política de Seguridad de Evil Hermes

Este documento describe el modelo de confianza de Evil Hermes, identifica el
...
o **security@hermes-agent.nousresearch.com**. No abras issues públicos para
...
Evil Hermes es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Evil Hermes,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Evil Hermes al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Evil Hermes
...
Evil Hermes admite dos posturas de aislamiento a nivel de SO. Abordan
...
Evil Hermes admite esto de dos maneras:

- **La propia imagen Docker de Evil Hermes y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Evil Hermes
...
Evil Hermes filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Evil Hermes
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Evil Hermes
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Evil Hermes no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Evil Hermes o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Evil Hermes ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Evil Hermes
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Evil Hermes. Los errores en la ruta de
  instalación de habilidades o plugins de Evil Hermes que impidan al
...
- Evil Hermes incluye guardias de cadena de suministro para lanzamientos de servidores
...
- **Canal:** el hilo GHSA o correspondencia por email con
  security@hermes-agent.nousresearch.com.
