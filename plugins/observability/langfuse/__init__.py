"""langfuse — Evil Hermes plugin for Langfuse observability.

Traces Evil Hermes conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Evil Hermes plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Evil Hermes plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Evil Hermes paths only expose ``task_id``/``session_id``. Newer paths
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Evil Hermes turn",
                tags=["hermes", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="Evil Hermes turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Evil Hermes branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Evil Hermes also has a turn-scoped
    ...
    # messages is a list only for legacy Evil Hermes branches that fired
    # pre_llm_call with API messages directly. Current Evil Hermes fires
...
def register(ctx) -> None:
    ...
    # Evil Hermes versions.  pre_api_request / post_api_request fire per API
