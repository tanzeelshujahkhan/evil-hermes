"""nemo_relay — optional Evil Hermes plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "Evil Hermes Agent"
...
class _Runtime:
    ...
    def _run_managed_with_downstream_preservation(
        ...
        *,
        ...
        # execution so Evil Hermes retry classification still sees it. The LLM and tool
...
def register(ctx) -> None:
    # Activate dynamic plugins before Evil Hermes installs the managed execution
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("HERMES_NEMO_RELAY_ATIF_AGENT_NAME") or "Evil Hermes Agent",
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Evil Hermes cannot activate Relay gateway [[plugins.dynamic]] records because "
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # Evil Hermes wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and Evil Hermes
