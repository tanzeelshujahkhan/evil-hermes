...
def cmd_sync(args) -> None:
    ...
    Scans all Evil Hermes profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Evil Hermes profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Evil Hermes persistent cross-session memory.")
    ...
    # These keys only affect the Evil Hermes GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Evil Hermes gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
...
def _active_profile_name() -> str:
    """Return the active Evil Hermes profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Evil Hermes' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Evil Hermes asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Evil Hermes + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Evil Hermes\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Evil Hermes persistent memory")
    ...
    if user_files:
        ...
        print("  (Evil Hermes calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Evil Hermes, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Evil Hermes:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Evil Hermes:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Evil Hermes:   Honcho observes every message and updates representations")
    ...
    print("    Evil Hermes:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Evil Hermes Honcho",
    )
