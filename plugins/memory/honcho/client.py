...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Evil Hermes profile."""
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Evil Hermes system prompt
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Evil Hermes session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Evil Hermes session title (from /title command; non-per-session)
