...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Evil Hermes does not currently need to route them into the agent loop.
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Evil Hermes handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Evil Hermes session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Evil Hermes owns. Do not write it into _channel_team: channel IDs can be
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Evil Hermes",
                bot_description="Your Evil Hermes agent on Slack",
            )
