...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Evil Hermes gateway
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Evil Hermes manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Evil Hermes pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Evil Hermes session")
        ...
        @tree.command(name="status", description="Show Evil Hermes session status")
        ...
        @tree.command(name="stop", description="Stop the running Evil Hermes agent")
        ...
        @tree.command(name="update", description="Update Evil Hermes Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Evil Hermes gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Evil Hermes session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Evil Hermes in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Evil Hermes skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Evil Hermes session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Evil Hermes: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Evil Hermes has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Evil Hermes"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Evil Hermes: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Evil Hermes semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Evil Hermes session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Evil Hermes handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Evil Hermes to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Evil Hermes needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Evil Hermes needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Evil Hermes could not create a Discord thread for "
