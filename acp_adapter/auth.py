"""ACP auth helpers — detect and advertise Evil Hermes authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Evil Hermes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Evil Hermes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Evil Hermes.
    ...
    not have Evil Hermes provider credentials configured yet, so Evil Hermes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Evil Hermes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Evil Hermes provider",
            description=(
                "Open Evil Hermes' interactive model/provider setup in a terminal. "
                "Use this when Evil Hermes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
