# Evil Hermes Security Policy

This document describes Evil Hermes' trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/tanzeelshujahkhan/evil-hermes/security/advisories/new)
or **security@hermes-agent.nousresearch.com**. Do not open public issues for
security vulnerabilities. **Evil Hermes does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Evil Hermes access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Evil Hermes' documentation
...
Evil Hermes supports two OS-level isolation postures. They address
...
Evil Hermes supports this in two ways:

- **Evil Hermes' own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Evil Hermes' in-process heuristics
...
Evil Hermes filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Evil Hermes
itself. Bugs in Evil Hermes' plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Evil Hermes does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Evil Hermes' own documentation, or reasonable
  operator expectations would predict — including cases where
  Evil Hermes has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Evil Hermes'
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Evil Hermes. Bugs in Hermes
...
- Evil Hermes includes supply-chain guards for MCP server
