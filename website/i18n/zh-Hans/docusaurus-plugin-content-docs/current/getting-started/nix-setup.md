...
执行 `nix profile install` 后，`hermes`、`evil-hermes` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`hermes setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
# /etc/nixos/flake.nix（或你的系统 flake）
{
  inputs = {
    ...
    hermes-agent.url = "github:TanzeelShujahKhan/evil-hermes";
  };
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `evil-hermes` 二进制文件存在且 `hermes version` 可运行 |
