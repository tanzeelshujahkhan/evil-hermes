...
| `hermes plugins` | 管理 Evil Hermes plugin（安装、启用、禁用、删除）。 |
...
- `evil-hermes` 代码本身（这是用户数据备份，不是仓库快照）。
...
拉取最新的 `evil-hermes` 代码并在受管理的 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。使用 `--check` 查看你的检出是否落后于 `origin/main`，而不安装。
