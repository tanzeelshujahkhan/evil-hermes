...
| `hermes plugins` | Manage Evil Hermes plugins (install, enable, disable, remove). |
...
Create a zip archive of your Hermes configuration, skills, sessions, and data. The backup excludes the evil-hermes codebase itself.
...
- The `evil-hermes` code itself (this is a user-data backup, not a repo snapshot).
...
Pulls the latest `evil-hermes` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.
