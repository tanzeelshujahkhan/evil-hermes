...
description: "Complete reference of all environment variables used by Evil Hermes"
---
...
| `API_SERVER_MODEL_NAME` | Model name advertised on `/v1/models`. Defaults to the profile name (or `evil-hermes` for the default profile). Useful for multi-user setups where frontends like Open WebUI need distinct model names per connection. |
