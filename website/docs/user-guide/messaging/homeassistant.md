...
description: Control your smart home with Evil Hermes via Home Assistant integration.
...
Evil Hermes integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Evil Hermes"
5. Copy the token
...
Evil Hermes registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Evil Hermes".
